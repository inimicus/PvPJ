...
local function RegisterCombatEvents()
	...
	if addon.inAvaWorld then
		...
		EVENT_MANAGER:AddFilterForEvent(addon.name, EVENT_COMBAT_EVENT, REGISTER_FILTER_IS_ERROR, false, REGISTER_FILTER_COMBAT_RESULT, ACTION_RESULT_KILLING_BLOW, REGISTER_FILTER_UNIT_TAG, COMBAT_UNIT_TYPE_PLAYER)
	elseif addon.inWorldBg then
		EVENT_MANAGER:RegisterForEvent(addon.name, EVENT_BATTLEGROUND_KILL, OnBattlegroundKill)
		EVENT_MANAGER:AddFilterForEvent(addon.name, EVENT_BATTLEGROUND_KILL, REGISTER_FILTER_UNIT_TAG, COMBAT_UNIT_TYPE_PLAYER)
	end
end
